local MobRenderer = {}
MobRenderer.__index = MobRenderer
...
function MobRenderer.new(model)
    local self = setmetatable({}, MobRenderer)
    self.modelName = model.Name
    self.box = {
        outer = NewBoxSet(Color3.fromRGB(0, 0, 0), 1),
        main  = NewBoxSet(Color3.fromRGB(255, 100, 100), 1),
...
function MobRenderer:UpdateBox(min, max, color)
    local o, i = 1, 1
    local sets = {
    ...
    for _, l in pairs(self.box.main) do
        l.Color = color or Color3.fromRGB(255, 100, 100)
    end
end

function MobRenderer:UpdateName(min, max)
    local fontSize = math.clamp(math.round((max.Y - min.Y) * 0.15), 13, 16)
...
function MobRenderer:HideName()
    self.nameText.Visible = false
end

function MobRenderer:UpdateHealthBar(min, max, character)
...
function MobRenderer:HideHealthBar()
    for _, l in pairs(self.healthBar) do l.Visible = false end
    self.healthText.Visible = false
end

function MobRenderer:HideBox()
...
function MobRenderer:Update(character, showBox, showHealth, showName, boxColor)
